...
function buildEvent(routeKey, connectionId, body) {
  const eventType =
...
async function forwardRoute(routeKey, connectionId, body) {
  if (!ROUTE_TARGET) {
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify(buildEvent(routeKey, connectionId, body)),
    });
